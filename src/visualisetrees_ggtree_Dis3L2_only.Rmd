...
This document is to analyse phylogenetic trees produced for Ssd1/Dis3L2 homologs.

Full raw sequences are in: `data/Dis3_families/Dis3L2_BLASTp_select_fullseqs.fasta`. 

File `data/Dis3L2_only/Dis3L2_BLASTp_select_fullseqs_mafftlocal_trimal_deltaNterm1to337.fasta` has the mafft-default-aligned, reordered, trimmed sequences. We ran IQ-TREE and RaxML on those files on the CIPRES server. This file analyses phylogenetic data from IQ-TREE results.
...
Dis3L2_features_df <- 
  ...
  read_tsv(comment="#", col_types = c("cccccliiiiiiiil"))%>%
...
ggt_taxa_IQML <- 
  ...
  dplyr::rename(Dis3L2_features_df,label=ID) +
    ...
    # geom_treescale(fontsize=3.6,y=2) +
    # geom_nodelab(size=1.8,nudge_x=0.04) + 
...
```{r plot percent_identity,dependson="calculate_percent_identity",fig.height=4,fig.width=4.5,echo=FALSE}
...
```{r percent_identity_cladogram,dependson="percent_identity",fig.height=4,fig.width=8,echo=FALSE}
percent_identity_cladogram <-
  plot_grid(ggtree(tree_IQML,branch.length="none"),
            plotclade_stripe + 
            ...
            plotdist_RNII   + 
              theme(legend.position=c(0.4, 0.1),
                    ...
                    plot.title = element_text(hjust = 0.5)),
            rel_widths=c(15,5,100,100),nrow=1,align="h")
percent_identity_cladogram

ggsave(filename = here::here("figure","figure3_percent_identity_cladogram.svg"), 
